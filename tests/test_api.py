...
def test_enqueue_launch_passes_cuda_visible_devices_from_slot(
    ...
    monkeypatch.setattr(jobs_router, "run_preflight", lambda *_a, **_k: [])
...
def test_create_job_inherits_settings_gpu_dispatch_default(
    client: TestClient,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_settings_persists_gpu_dispatch_defaults(client: TestClient) -> None:
    r = client.put(
        "/api/settings",
        json={
            "max_concurrent_jobs": 2,
            "gpu_dispatch_mode": "distributed",
            "gpu_dispatch_num_gpus": 2,
        },
    )
    assert r.status_code == 200, r.text
