...
- **React web workbench** — Vite + React frontend with dashboard, job monitor, recipe browser/editor, and settings pages. The FastAPI server serves the built SPA from `web/dist` when present, while all API routes stay under `/api`.
- **Recipe validate/save API** — `/api/recipes/validate` returns structured Pydantic field errors for the visual editor, and `/api/recipes` persists validated recipes to `recipes/<name>.yaml` with overwrite protection and path traversal checks.
- **Workbench settings API** — `/api/settings` reads and writes user-level defaults for kohya `sd-scripts`, Python executable, and WD14 tagger device. `/api/health` includes the same backend probe so the UI can show setup status.
- **SQLite-backed job persistence** — `lorahub.api.store.JobStore` mirrors job metadata (id, state, recipe snapshot, timestamps, returncode, pid) to a SQLite file at `runs/.lorahub.sqlite`. The API server loads existing jobs on startup and converts any non-terminal jobs (`queued`/`running`/`canceling`) to a new `interrupted` state, since their processes did not survive the restart. `JobRegistry.update()` mirrors live state changes back to disk. WAL mode is enabled for write throughput. Live `TrainingHandle` and the per-job event ring buffer remain in-memory only — `events.jsonl` already provides on-disk replay.
- **HTTP API (v0.2 starter)** — `lorahub.api` module exposing FastAPI routes under `/api` for health, settings, recipe schema/list/detail/validate/save, jobs (CRUD), `/api/jobs/{id}/events`, and `/api/jobs/{id}/stream` (WebSocket). In-process `JobRegistry` keeps live state and a per-job event ring buffer so reconnecting WebSocket clients can replay. Launch with `lorahub serve [--host ... --port ... --reload]`. Install API extras: `pip install lorahub[api]`.
