...
function UploadDropZone({
  ...
  const isArchive = (name: string) => /\.(zip|tar|tar\.gz|tgz|7z|rar|gz)$/i.test(name)
  ...
  const clearDone = () => setQueue((prev) => prev.filter((t) => t.status !== "done" && t.status !== "error"))
      ...
      <div className="px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            {doneTasks.length > 0 && (
              <button type="button" onClick={clearDone} className="text-xs text-muted-foreground hover:text-foreground">
                清除已完成
              </button>
            )}
            <button
            ...
            {queue.map((task) => (
              <div key={task.id} className="flex items-center gap-2 text-xs">
                <span className={`size-2 rounded-full shrink-0 ${
                  ...
                  task.status === "uploading" || task.status === "extracting" ? "bg-primary animate-pulse" :
