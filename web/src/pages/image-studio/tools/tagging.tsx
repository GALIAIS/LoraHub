/**
 * 打标类工具 — WD14 自动打标 / 标签词频 / 找替换 / 注入触发词 / 标签黑名单。
 *
 * 词频 / 找替换 / 注入触发词 全部复用 annotate-stage 里已抽出的 *Tool 包装器。
 * WD14 直接复用 TaggingPanel；黑名单 (按列表批量删除 tag) 单独写，UI 比
 * VocabPanel 的 hover 删除更直接。
 */
...
import {
  FindReplaceTool as AnnotateFindReplaceTool,
  TriggerInjectTool as AnnotateTriggerInjectTool,
  VocabPanelTool as AnnotateVocabPanelTool,
} from "../components/stages/annotate-stage"

// --------------------------------------------------------------------------- //
// tagging-wd14
// --------------------------------------------------------------------------- //

export function TaggingWd14Tool({ datasetPath }: { datasetPath: string }) {
  return (
    <div className="h-full overflow-y-auto p-4 max-w-2xl">
      <section className="rounded-md border border-border/60 bg-card">
        <TaggingPanel datasetPath={datasetPath} />
      </section>
    </div>
  )
}
...
// --------------------------------------------------------------------------- //
// captions-vocab / captions-find-replace / captions-inject-trigger
// 直接转发到 annotate-stage 的 *Tool 包装器。
// --------------------------------------------------------------------------- //

export const CaptionsVocabTool = AnnotateVocabPanelTool
export const CaptionsFindReplaceTool = AnnotateFindReplaceTool
export const CaptionsInjectTriggerTool = AnnotateTriggerInjectTool
