...
import { api, type DatasetScanResponse, type FsEntry } from "@/lib/api"
...
export function DatasetsPage() {
  ...
  const [editor, setEditor] = useState<
    | { kind: "fs"; path: string }
    | { kind: "caption"; imagePath: string; path: string }
    | null
  >(null)
  const [selectedFsPath, setSelectedFsPath] = useState<string | null>(null)
  ...
  const onFsFolder = (folder: string) => {
    setSelectedFsPath(folder)
  }
  const onFsFileSelect = (entry: FsEntry) => {
    setSelectedFsPath(entry.path)
  }
  const onFsFileOpen = (entry: FsEntry) => {
    setSelectedFsPath(entry.path)
    if (entry.kind === "binary") return
    setEditor({ kind: "fs", path: entry.path })
  }

  return (
    <div className="h-full flex min-h-0">
      <aside className="w-72 shrink-0 border-r border-border/60 bg-sidebar/40 flex flex-col min-h-0">
        <FileBrowser
          selectedPath={selectedFsPath}
          onSelectFolder={onFsFolder}
          onSelectFile={onFsFileSelect}
          onOpenFile={onFsFileOpen}
        />
        {selectedFsPath && (
          <div className="border-t border-border/60 p-2 flex flex-col gap-1.5">
            <div className="text-[10.5px] uppercase tracking-[0.18em] text-muted-foreground/85">
              当前选中
            </div>
            <div className="font-mono text-[11px] break-all text-muted-foreground line-clamp-3">
              {selectedFsPath}
            </div>
            <div className="flex gap-1.5 mt-1">
              <Button
                ...
                onClick={() => navigateTo(selectedFsPath)}
              >
                作为数据集扫描
              </Button>
            </div>
        ...
        <div className="px-8 py-7 space-y-5 w-full">
          <header className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              左侧浏览器可直接打开任意文件编辑；右侧扫描目录、预览缩略图、点击「编辑」弹窗修改 caption。
            </p>
          </header>
                    ...
                    <SampleGallery
                      ...
                      onEdit={(sample: Sample) =>
                        setEditor({
                          ...
                          path: sample.path.replace(/\.[^.]+$/, ".txt"),
                        })
                      }
                      onPreviewImage={(sample) =>
                        setEditor({ kind: "fs", path: sample.path })
                      }
                    />
                  )}
                </CardContent>
