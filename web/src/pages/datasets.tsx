...
function TaggingDialog({
  ...
  // Seed device from saved settings the first time settings load.
  useEffect(() => {
  ...
  // When the run finishes, refresh the dataset scan so caption coverage updates.
  useEffect(() => {
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-1">
              <Button
                ...
                size="sm"
                onClick={() => {
                  // Run-again: clear the session so the form comes back.
                  setSessionId(null)
                  start.reset()
                }}
                disabled={running}
